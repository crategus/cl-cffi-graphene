...
(test with-ray.4
  (graphene:with-point3d (p)
    (graphene:with-vec3 (v)
      (graphene:with-rays (ray (ray1 ray)
...
(test ray-init-from-vec3
  (graphene:with-vec3s ((v1 1 2 3) (v2 4 5 6))
    (graphene:with-ray (ray)
      (is (cffi:pointerp ray))
...
(test ray-origin/direction
  (graphene:with-point3ds ((origin 1.5 2.5 3.5) p)
    (graphene:with-vec3s ((direction 1.0 0.0 0.0) v)
      (graphene:with-ray (ray)
